...
pub fn handle_command_logout<'a>(data:&Bytes,command:&'a CommandDataGram) -> Option<BitcommDataGram<'a>> {
...
pub fn process_command_logout(command:&CommandDataGram,
                            stmid   :u64,
    ...
    // 
    let mut ccp = cpm.lock();
    // 缓存client的信息
    // ccp.remove_client(command.sender().into(), command.deviceid());
